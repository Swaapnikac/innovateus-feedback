...
| `qualtrics_sync_success_rate` | S8 | Data-pipeline health (resolved per-cohort target — production / test) | `subs with qualtrics_synced_at IS NOT NULL / completed` | Qualtrics card | `user-testing.csv.qualtrics_sync_success_flag`, `raw.csv.qualtrics_*`, `qualtrics.csv` |
